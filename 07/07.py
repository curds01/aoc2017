...
class Tower:
    def __init__(self, name, weight, child_towers):
        self.name = name
        self.weight = weight
        self.child_towers = child_towers
        self.parent = None

    def __str__(self):
        s = repr(self)
        if (self.child_towers):
            s += ' -> %s' % ', '.join(map(lambda x: x.name, self.child_towers))
        return s
...
def parse_tower(line):
    ...
    if (len(tokens) < 4):
        return Tower(tokens[0], int(tokens[1][1:-1]), [])
    else:
        name = tokens[0]
        weight = int(tokens[1][1:-1])
        children = map(lambda x: x.strip(','), tokens[3:])
        return Tower(name, weight, children)

towers = map(parse_tower, data.split('\n'))
towers = dict((t.name, t) for t in towers)

parents = set([t for t in towers.values() if t.child_towers])
for p in parents:
    for i, child_name in enumerate(p.child_towers):
        child = towers[child_name]
        child.parent = p
        p.child_towers[i] = child
root = filter(lambda x: not x.parent, towers.values())[0]
print root.name, 'is the root'
print

def calc_weight(tower, indent=0):
    '''Compute the weight of the tower'''
    weight = tower.weight
    child_weights = {}
    for child in tower.child_towers:
        w = calc_weight(child, indent + 1)
        if (child_weights.has_key(w)):
            child_weights[w].append(child)
        else:
            child_weights[w] = [child]
    if (len(child_weights) > 1):
        bad_value = filter(lambda pair: len(pair[1]) == 1, child_weights.items())[0]
        good_value = filter(lambda pair: len(pair[1]) > 1, child_weights.items())[0]
        delta = good_value[0] - bad_value[0]
        print 'Tower "%s" has uneven children' % tower.name
        print '\t', tower
        print "Tower %s needs its weight changed from %d to %d" % (bad_value[1][0].name, bad_value[1][0].weight, bad_value[1][0].weight + delta)
        sys.exit(0)
    if (len(child_weights) > 0):
        weight += child_weights.keys()[0] * len(child_weights.values()[0])
    return weight
calc_weight(root)

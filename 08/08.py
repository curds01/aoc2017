...
def read_memory(register):
    global memory
    if (memory.has_key(register)):
        return memory[register]
    else:
        memory[register] = 0
        return 0

def add_memory(register, value):
    if (memory.has_key(register)):
        memory[register] += value
    else:
        memory[register] = value

f = eval('lambda x, y: x %s y' % '==')

class Condition:
    def __init__(self, op1, op, op2):
##        print "Condition(%s, %s, %s)" % ( op1, op, op2)
        self.reg = op1
        self.op_str = op
        self.op = eval('lambda x, y: x %s y' % op)
        self.value = int(op2)
    ...
    def is_true(self):
        return self.op(read_memory(self.reg), self.value)

    def __str__(self):
        return '%s %s %s' % ( self.reg, self.op_str, self.value)
...
def parse_instruction(line):
    ...
    condition = Condition(cond[0], cond[1], cond[2])
    return (reg, dir, value, condition)

def execute(reg, value, condition):
    print reg, value, condition,
    if condition.is_true():
        print read_memory(reg), '-->',
        add_memory(reg, value)
        print read_memory(reg)
    else:
        print
...
biggest = 0
for tgt, dir, value, condition in instructions:
    # always add
    if (dir == 'dec'): value = -value
    execute(tgt, value, condition)
    biggest = max(memory.values() + [biggest])
    
    
biggest_value()
print "Ever biggest", biggest
